...
rm(list=setdiff(setdiff(setdiff(ls(), "newExpectedValues"), "expectedValues"), "listOfTanks")) #remove all but a few
...
for (i in listOfTanks$compDescr) {
	...
	sample <- as.data.frame(dbGetQuery(con, sprintf('SELECT * FROM stats WHERE compDescr = %i', i)));
