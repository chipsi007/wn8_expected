...
tempData <- read.csv.ffdf(file="input.csv", header=TRUE, VERBOSE=TRUE, first.rows=10000, next.rows=1000000)
dataMaster <- as.data.frame(tempData)
...
#write.csv(x=userTankStatsFiltered,file="temp.csv" ,row.names = FALSE)
require(data.table)
userTankStatsFiltered <- data.table(userTankStatsFiltered)
rm(list=setdiff(setdiff(setdiff(setdiff(ls(), "newExpectedValues"), "expectedValues"), "listOfTanks"), "userTankStatsFiltered")) #remove all variables but a few

#write.csv.ffdf(as.ffdf(userTankStatsFiltered), file = "temp.csv", VERBOSE=TRUE, first.rows=10000, next.rows=1000000);
#con <- dbConnect(RSQLite::SQLite(), dbname = "stats.sqlite")
#dbWriteTable(con, name="stats", value="temp.csv", row.names=FALSE, header=TRUE, sep = ",")

userTankStatsFiltered <- split(userTankStatsFiltered, userTankStatsFiltered$compDescr)

for (j in 1:length(userTankStatsFiltered)) {
	sample <- as.data.frame(userTankStatsFiltered[[j]]);
	i <- sample[1,"compDescr"]
