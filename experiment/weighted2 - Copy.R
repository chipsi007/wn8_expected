...
rm(list=setdiff(ls(), "newExpectedValues")) #remove all variables but newExpectedValues

for (i in listOfTanks$compDescr) {
    #sample <- userTankStatsFiltered[userTankStatsFiltered$compDescr == i,]
	sample <- read.csv.sql("temp.csv", sql = sprintf("select * from file where compDescr == %i;", i);
