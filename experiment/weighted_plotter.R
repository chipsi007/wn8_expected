...
#####################################
    jpegString <- paste("plots/", names(model$model)[1],"_vs_",names(model$model)[2], "_v31_%i_%s.jpg")
    ggplotRegression(model, listOfTanks$title[listOfTanks$compDescr == i] )
    savedPlot <- sprintf(jpegString, listOfTanks$compDescr[listOfTanks$compDescr == i], gsub(":", "-", listOfTanks$title[listOfTanks$compDescr == i]))
