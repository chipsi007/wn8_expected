...
require(data.table)
dataMaster <- fread("input.csv")
dataMaster <- as.data.frame(dataMaster)
gc()
...
rm(list=setdiff(setdiff(setdiff(setdiff(setdiff(ls(), "newExpectedValues"), "expectedValues"), "listOfTanks"), "userTankStatsFiltered"), "userAccountStats") #remove all variables but a few

userTankStatsFiltered <- na.omit(userTankStatsFiltered)
options(scipen=999)
write.csv(userTankStatsFiltered, "temp.csv");
rm(userTankStatsFiltered)
gc()
...
dbWriteTable(con, name="stats", value="temp.csv", header=TRUE)
