...
write.csv(x=listOfTanks,file="tankList.csv",row.names = FALSE)

# Throw away the workspace and load everything we need in  a more efficient way
rm(list = ls())

db <- dbConnect(SQLite(), dbname="temp.sqlite")
dbWriteTable(conn = db, name = "temp", value = "temp.csv", row.names=FALSE, header=TRUE, eol="\n", sep=",")
...
for (i in listOfTanks$compDescr){
  #sample <- userTankStatsFiltered[userTankStatsFiltered$compDescr == i,]
	#sample <- filter(userTankStatsFiltered, compDescr == i)
