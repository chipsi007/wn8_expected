...
for (i in listOfTanks$compDescr){
	...
	if (nrow(sample) > 0) {	
		...
		model <- lm(rDAMAGEc ~ user_rDAMAGEc, data=sample, weights=battles)
		#####################################
		jpegString <- paste("weighted_plots/", names(model$model)[1],"_vs_",names(model$model)[2], "_v31_%i.jpg")
...
for (i in listOfTanks$compDescr){
	...
	if (nrow(sample) > 0) {	
		######set model parameters###########
		model <- lm(WN8 ~ user_WN8, data=sample, weights=battles)
		#####################################
		jpegString <- paste("weighted_plots/", names(model$model)[1],"_vs_",names(model$model)[2], "_v31_%i.jpg")
		ggplotRegression(model, listOfTanks$title[listOfTanks$compDescr == i] )
		savedPlot <- sprintf(jpegString, listOfTanks$compDescr[listOfTanks$compDescr == i])
...
savedPlot <- paste("weighted_plots/",names(usermodel$model)[1],"vs", names(usermodel$model)[2],"_v31.jpg")
ggsave(file=savedPlot, width=7, height=7)